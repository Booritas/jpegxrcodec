...
void testJxrFileDecoder(const std::string& jxrImageName, const std::string& rawImageName, int imageWidth, int imageHeight, int imageChannels)
{
...
{
    testJxrFileDecoder("seagull.wdp", "seagull.raw", 1000, 667, 3);
}

TEST(jxrcodec, decodeFileJxr)
{
    testJxrFileDecoder("sample.jxr", "sample.raw", 128, 128, 3);
}

void testJxrMemDecoder(const std::string& jxrImageName, const std::string& rawImageName, int imageWidth, int imageHeight, int imageChannels)
{
    const int imageSize = imageWidth*imageHeight*imageChannels;
    std::vector<uint8_t> raster(imageSize);
    std::string imagePath = getTestImagePath(jxrImageName);
...
{
    testJxrMemDecoder("seagull.wdp", "seagull.raw", 1000, 667, 3);
}

TEST(jxrcodec, decodeBufferJxr)
{
    testJxrMemDecoder("sample.jxr", "sample.raw", 128, 128, 3);
}

TEST(jxrcodec, decodeBufferJxr2)
{
    testJxrMemDecoder("test.jxr", "test.raw", 1920, 1080, 3);
}

TEST(jxrcodec, decodeBufferJxrTissue)
{
    testJxrMemDecoder("tissue.jxr", "tissue.raw", 550, 345, 3);
}
